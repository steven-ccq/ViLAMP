...
class ModelArguments:
    ...
    # Frame Selector
    clip_model_path: Optional[str] = field(default="clip-ViT-B-32/0_CLIPModel")
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ["mm_projector", "vision_tower", "vision_resampler", "composer_projector"]
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            query = self.list_data_dict[i].get('query', None)
            max_frame_num = self.list_data_dict[i].get('max_frame_num', 32)
            video_folder = self.data_args.video_folder
            if isinstance(video_file, str):
                video_file = [os.path.join(video_folder, video_file)]
            elif isinstance(video_file, list):
                video_file = [os.path.join(video_folder, v) for v in video_file]
            ...
            try:
                ...
                for v in video_file:
                    if "shareVideoGPTV" in v:
                    ...
                    elif "VideoChatGPT" in v:
                        video += self.access_video_by_sec(v)
                    elif "finevideo" in v:
                        video += self.access_video_by_sec(v)
                    elif "llava-video" in v:
                        video += self.access_video_by_sec(v)
                    elif "ShareGPT4Video" in v:
                        video += self.access_video_by_sec(v)
                    elif "ShareGPTVideo" in v:
                        ...
                        for file in frame_files:
                            try:
                                ...
                                continue
                        if len(v_frames) > 600:
                            sampled_indices = np.linspace(0, len(v_frames)-1, 600, dtype=int)
                            v_frames = [v_frames[i] for i in sampled_indices]
                        video += v_frames
                    elif "videoInstruct" in v:
                        v_frames = self.access_video_by_sec(v)
                        if len(v_frames) > 600:
                            sampled_indices = np.linspace(0, len(v_frames)-1, 600, dtype=int)
                            v_frames = [v_frames[i] for i in sampled_indices]
                        video += v_frames
                ...
                if query is not None:
                    clip_processor = self.data_args.clip_processor
                    max_query_embeddings = self.data_args.max_query_embeddings
                    image_inputs = clip_processor(images=video, return_tensors='pt')
                    query_inputs = clip_processor(text=[query], truncation=True, max_length=max_query_embeddings, return_tensors='pt')
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
            except Exception as e:
                print(f"Error: {e}")
        ...
        if query is not None:
            data_dict['image_inputs'] = image_inputs
            data_dict['query_inputs'] = query_inputs

        return data_dict

    def access_video_by_sec(self, video_path):
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # Frame Selector
        if "image_inputs" in instances[0] and "query_inputs" in instances[0]:
            batch["image_inputs"] = [instance["image_inputs"] for instance in instances]
            batch["query_inputs"] = [instance["query_inputs"] for instance in instances]
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        # Frame Selector
        data_args.clip_processor = transformers.CLIPProcessor.from_pretrained(model_args.clip_model_path)
        data_args.max_query_embeddings = transformers.CLIPTextConfig().max_position_embeddings
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            rank0_print(f"Ignore mm_tunable_parts: {model_args.mm_tunable_parts}. Training LoRA modules only")
            ...
            # Set Frame Selector too
            model.get_model().frame_selector.requires_grad_(False)
            # Also set Composer Projector
            model.get_model().composer_projector.requires_grad_(False)
            ...
            model.get_model().vision_resampler.requires_grad_(False)
            # Set Frame Selector too
            if getattr(model.get_model(), 'frame_selector', None):
                model.get_model().frame_selector.requires_grad_(False)
            # Also set Composer Projector
            if getattr(model.get_model(), 'composer_projector', None):
                model.get_model().composer_projector.requires_grad_(False)
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name and "frame_selector" not in name and "composer_projector" not in name:
                        param.requires_grad_(True)
            if "frame_selector" in tunable_parts:
                for p in model.get_model().frame_selector.parameters():
                    p.requires_grad = True
            if "composer_projector" in tunable_parts:
                for p in model.get_model().composer_projector.parameters():
                    p.requires_grad = True
            if "pre_fusion_model" in tunable_parts:
                for p in model.get_model().pre_fusion_model.parameters():
                    p.requires_grad = True
