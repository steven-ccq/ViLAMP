...
class SigLipVisionTower(nn.Module):
    ...
    def forward(self, images, max_dense_length=60):
        if type(images) == torch.Tensor and images.shape[0] > max_dense_length:
            images = list(torch.unbind(images, dim=0))

        if type(images) is list:
            image_features = []
            for idx in range(0, len(images), max_dense_length):
                image = torch.stack(images[idx:min(idx+max_dense_length, len(images))], dim=0)
