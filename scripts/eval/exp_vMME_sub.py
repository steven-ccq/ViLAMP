...
{question}.
Please think carefully and choose the best answer from the following options:
{options}
The answer is: """
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--subtitle_dir', default='dataset/Video-MME/subtitle')
    ...
    args.output_file = os.path.join(args.output_dir, '{}-{}_sub.json'.format(args.version, args.split))
    ...
    with open(args.output_file, 'w') as f:
        for data in tqdm(dataset):
            ...
            try:
                subtitle_path = os.path.join(args.subtitle_dir, '{videoID}.srt'.format(videoID=videoID))
                with open(subtitle_path, 'r') as srt:
                    subtitle = srt.read()
                subtitle = find_sub(subtitle)
            except Exception as e:
                print(e)
                subtitle = ''
            query = prompt.format(subtitle=subtitle, question=question, options=options)
            try:
